...
namespace TestProjectBBC.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class DevelopController : ControllerBase
    {
        private readonly IUserService _userService;   // DI
        ...
        public async Task<IActionResult> GetToken(int id = 1)
        {
            ...
            {
                return Unauthorized(new { Message = "No faund user with this ID" });
            }
        }
        ...
        {
            return Ok(new { Message = "Message for admin only" });
        }
